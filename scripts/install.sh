...
if [[ "${INSTALL_WHEEL}" == "1" ]]; then
  ...
  if command -v patchelf >/dev/null 2>&1; then
    ...
    for so in "${TORCH_PREFIX}"/lib/python*/site-packages/pymomentum/*.so; do
      if [[ -f "${so}" ]]; then
        if [[ -d "${DEPS_PREFIX}" ]]; then
          patchelf --set-rpath "${DEPS_PREFIX}/lib:${TORCH_PREFIX}/lib:\$ORIGIN/../torch/lib" "${so}" 2>/dev/null || true
        else
          patchelf --set-rpath "${TORCH_PREFIX}/lib:\$ORIGIN/../torch/lib" "${so}" 2>/dev/null || true
        fi
      fi
    done
  fi
